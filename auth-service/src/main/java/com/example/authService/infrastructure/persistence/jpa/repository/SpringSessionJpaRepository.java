...
public interface SpringSessionJpaRepository extends JpaRepository<SessionJpaEntity, UUID> {
    ...
    @Query("""
            ...
            set session.replacedBy = :newSessionId,
                session.status = com.example.authService.domain.enums.SessionStatus.REPLACED,
                session.revokedAt = :now
            where session.refreshTokenHash = :oldTokenHash
              and session.status = com.example.authService.domain.enums.SessionStatus.ACTIVE
            """)
    ...
    @Query("""
            update SessionJpaEntity session
            set session.status = com.example.authService.domain.enums.SessionStatus.REVOKED,
                session.revokedAt = :now
